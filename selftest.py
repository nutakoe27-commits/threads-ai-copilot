...
def main() -> int:
    ...
    check("в оффере есть просьба про удачного клиента",
          "вот таких бы побольше" in offer_text)
    check("в оффере есть, что Михаил даёт в ответ",
          "список из пяти компаний" in offer_text)
    ...
    good = (
        ...
        "Если назовёте одного-двух заказчиков за последний год, которыми вы "
        "довольны, я соберу и пришлю пять похожих компаний с обоснованием. "
    ...
    _, notes = compose.finish_letter(good.replace("Бесплатно", "Бесплатно!"), origin)
