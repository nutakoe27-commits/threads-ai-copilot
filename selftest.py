...
def main() -> int:
    ...
    check("бриф задаёт длину по данным, а не на глаз",
          "90–150 слов" in letter_frame and "Больше 180 — не бывает" in letter_frame)
    check("оговорка про структуру важнее числа на месте",
          "структура весит больше объёма" in letter_frame)
    check("бриф описывает, что читатель сканирует",
          "он **сканирует**" in letter_frame)
    check("бриф требует суть в начале",
          "сначала вывод, потом" in letter_frame)
    check("бриф запрещает решать за читателя",
          "Выводов за читателя" in letter_frame)
    check("бриф ограничивает тему 50 знаками",
          "Не длиннее 50 знаков" in letter_frame)
    check("бриф ограничивает абзац тремя строками",
          "не длиннее трёх строк" in letter_frame)
    check("работа в одиночку подана как преимущество",
          "собственник разговаривает с собственником" in letter_frame)
    check("вакансия названа самым весомым приёмом",
          "начинай с неё, всегда" in letter_frame)
